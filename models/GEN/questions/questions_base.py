...
class Questao(BaseInfoModel):
    def __init__(self
        ...
        self.respostas_multiplas = respostas_multiplas
        ...
        if preposicao is not None: 
            ...
            if respostas_multiplas == True:
                ...
                self.preposicao = preposicao + f" Retorne o resultado em uma lista JSON conforme o seguinte modelo de dados: {data_model_preposicao}"
            else:
    ...
    def __datamodel_to_values_class(self):
        ...
        def _validacao_list(obj: list, chave: str):
            if len(obj) == 0:
                raise Exception(f"'Questao.data_model (dict)' suporta apenas listas com pelo menos um elemento da classe 'Especificacao_da_Resposta'. Chave '{k}'")
            
            for k in obj:
                if not k.__class__ == Especificacao_da_Resposta:
                    raise Exception(f"Todos os items listas em 'Questao.data_model (dict)' devem ser da classe 'Especificacao_da_Resposta'. Chave '{k}'")
        ...
        molde["data_model"] = self.data_model #self.data_model é copiado para orientacao das regras de preenchimento

        print(molde)

        ValuesClass = create_dynamic_class(class_name, molde, (BaseValueClass,))
